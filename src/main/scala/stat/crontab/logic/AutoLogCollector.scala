...
object AutoLogCollector extends Utils {
	...
	/** @return time -> (k,v,c) */
	def parse(lines: Seq[String]): Map[String, Seq[(String, Double, Int)]] = {
		// (201311032304, crawl.cost, 1147.567469879518, 1660)
		val rows: Seq[(Long, String, Double, Int)] = lines.map(line => line.trim match {
			case log_line_pattern(minute, k, v, c) => (minute.toLong, k, v.toDouble, c.toInt)
			case _ => null
		}).filter(_ != null)
		...
		rows.groupBy(row => (row._1 - row._1 % 5).toString).mapValues(t_rows => {
			t_rows.groupBy(_._2).mapValues(k_rows => {
				val sum: Double = k_rows.map(row => row._3 * row._4).sum
				val c: Int = k_rows.map(_._4).sum
				(sum / c, c)
	...
	private val log_line_pattern = "(\\d{12})\\s+(\\S{1,80})\\s+avg:(\\S+)\\s+count:(\\d+)".r
